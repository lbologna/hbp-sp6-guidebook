...
This usecase places hippocampal neurons into a volume defined by an atlas.
The use case can be found in *Online Use Cases/Circuit Building/Cells placement/Hippocampus CA1*.
...
The workflow is a sequence of independent steps manipulating volumetric data retrieved from the neuroinformatics platform. Each steps define additional properties to the cells.
The sequence of steps is the following:
 ...
 - Define morphology to use based on available morphologies and previously defined properties
...
We collected 146 electrophysiological single cell recordings from different neuron types (Alex Thomson, Audrey Mercer and co)  (Wistar rat - for the methods see `Hughes et al 2000 <http://www.sciencedirect.com/science/article/pii/S0165027000002545>`_; `Pawelzik et al 2002 <http://onlinelibrary.wiley.com/doi/10.1002/cne.10118/abstract>`_). We classified the recordings on the basis of stereotypical firing patterns (electrical types, e-types) described in Petilla convention `(Ascoli et al 2008) <https://www.ncbi.nlm.nih.gov/pubmed/18568015>`_ in response to depolarizing step current injections. For some e-types, we had too few recordings and for this reason we restricted the e-types to classical accommodating (cAC), bursting accommodating (bAC) and classical non-accommodating (cNAC). Each morphology type (m-type) can show multiple firing patterns, leading to different morpho-electrical types (me-types).
...
1. positioning neurons
   ...
   This step computes cells positions with downscaled densities as an illustration. Then it displays the cell positions in a widget.

      .. image:: images/position.png
         :width: 600px
2. defining excitatory/inhibitory property

   This step computes the excitatory/inhibitory property based on provided ratio and displays it in a widget.
...
4. defining morphology for the cell position
   ...
   This steps computes which morphology will be assigned to a particular cell position.
...
5. defining cell orientation
   ...
   This steps computes the 3 vector fields X, Y, and Z and use them to orient each neuron.
